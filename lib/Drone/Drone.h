...
private:
  ...
  MotorInterface::MotorCommand motor_command;
  double imuValues[12];
...
public:
  Drone(MotorInterface *mc, Receiver *r);
  ...
  MotorInterface::MotorCommand getMotorValues();

  void printCalibrationStatus();

  uint8_t getSystemCalibration() {
    return sensor ? sensor->getSystemCalibration() : 0;
  }
  uint8_t getGyroCalibration() {
    return sensor ? sensor->getGyroCalibration() : 0;
  }
  uint8_t getAccelCalibration() {
    return sensor ? sensor->getAccelCalibration() : 0;
  }
  uint8_t getMagCalibration() {
    return sensor ? sensor->getMagCalibration() : 0;
  }
};
